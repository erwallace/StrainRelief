...
@pytest.mark.gpu
@pytest.mark.parametrize("model_path_fixture", ["esen_model_path"])
def test_predict_energy_nnp(mols: dict[str, Chem.Mol], model_path_fixture: str, request):
    ...
    result = predict_energy(mols, "eSEN", **kwargs)
    assert result is not None
    assert isinstance(result, dict)
    assert len(result) == len(mols)
